...
TEST(FactoryTest, CreateWillianCorrectly) {
    auto npc = Factory::CreateNPC("Willian", "Robin", 0, 500);
    ASSERT_TRUE(npc != nullptr);
    EXPECT_EQ(npc->type, WillianType);
}

TEST(FactoryTest, CreateWerewolfCorrectly) {
    auto npc = Factory::CreateNPC("Werewolf", "Lucian", 250, 250);
    ASSERT_TRUE(npc != nullptr);
...
TEST(FactoryTest, ThrowOnUnknownType) {
    EXPECT_THROW(Factory::CreateNPC("Dragon", "Smaug", 100, 100), std::runtime_error);
}
...
TEST(FactoryTest, ThrowOnOutOfBoundsCoordinates) {
    EXPECT_THROW(Factory::CreateNPC("Ork", "Bad", 501, 50), std::runtime_error);
    EXPECT_THROW(Factory::CreateNPC("Ork", "Bad", 50, 501), std::runtime_error);
}

TEST(FactoryTest, CreateFromStream) {
    std::stringstream ss;
    ss << "Ork 100 200 Guldan";
...
TEST(NPCTest, DistanceCalculationFar) {
    auto n1 = std::make_shared<Ork>(0, 0, "A");
    auto n2 = std::make_shared<Ork>(100, 100, "B");
...
TEST(ArenaTest, FightNoRange) {
    Arena arena;
    auto n1 = std::make_shared<Ork>(0, 0, "A");
    auto n2 = std::make_shared<Willian>(100, 100, "B");
